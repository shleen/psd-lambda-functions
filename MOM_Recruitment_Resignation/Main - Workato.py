# Motivation: 
# Recruitment rate: The recruitment rate is useful as another data source to compare for 
# output of tech talent. 
# Resignation rate: The resignation rate is useful to compare and see if there is a higher
# than normal resignation rate for the Infocomm sector. 

import requests
import pandas as pd
import datetime as dt

final_filename = "MOM_RnR.xlsx"
...
# write the dataframes into an excel file with 2 different sheet names
...
# else can also write into 2 separate csv files
# rec.to_csv("MOM_Recruitment_Rate.csv", index=False)
# resg.to_csv("MOM_Resignation_Rate.csv", index=False)

# os.remove(rec_filename)
# os.remove(resg_filename)